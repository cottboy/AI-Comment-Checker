...
// 删除插件选项
delete_option( 'ai_comment_checker_settings' );
...
// 清除定时任务
wp_clear_scheduled_hook( 'ai_comment_checker_cleanup_logs' );
...
$wpdb->query(
    "DELETE FROM {$wpdb->options} WHERE option_name LIKE '_transient_ai_comment_checker_%' OR option_name LIKE '_transient_timeout_ai_comment_checker_%'"
);
...
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- 卸载时清理数据，options 表引用安全
$wpdb->query(
    "DELETE FROM {$wpdb->options} WHERE option_name LIKE 'ai_comment_checker_%'"
);
